...
const reportAPI = {
  ...
  getBudgetForecast: async (flatId, params = {}) => {
    try {
      ...
      const res = await api.get(`/v2/reports/flats/${flatId}/forecast`, { params });
  ...
  getCategorySpending: async (flatId, params = {}) => {
    try {
      ...
      const res = await api.get(`/v2/reports/flats/${flatId}/categories`, { params });
  ...
  /**
   * Export report as PDF (backend generates PDF using PDFKit)
   * @param {string} flatId - Flat ID
  ...
  exportReport: async (flatId, params = {}) => {
    try {
      console.log('💾 Exporting report for flat:', flatId, 'with params:', params);
      
      // Request PDF format from backend (binary stream)
      ...
      // Check if response is actually an error (JSON in arraybuffer)
      if (res.status >= 400) {
      ...
      console.log('✅ Received PDF data:', res.data.byteLength, 'bytes');
      ...
      console.log('✅ PDF report saved to:', fileUri);
      ...
      console.error("❌ Export report error:", error.response?.data || error.message || error);
